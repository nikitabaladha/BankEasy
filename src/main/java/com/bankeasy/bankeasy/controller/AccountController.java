...
public class AccountController {
    ...
    @PostMapping("/create")
    public ResponseEntity<ApiResponse<Account>> createAccount(@Valid @RequestBody AccountCreateRequest request, BindingResult result) {
        if (result.hasErrors()) {
            return new ResponseEntity<>(new ApiResponse<>(true, "Validation error: " + result.getAllErrors().get(0).getDefaultMessage(), null), HttpStatus.BAD_REQUEST);
        }
